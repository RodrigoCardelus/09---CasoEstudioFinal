...
Begin
       --valido FK
        ...
        --valido PK
        if(exists(select * from TelefonosCliente where NumCli = @NumCli AND TelCli = @TelCli))
           return -3
...
Begin
		...
		--valido PK
		if(NOT exists(select * from CuentaCorriente where NumCta = @NumCta))
		        return -5
...
Begin
		...
		Begin
		   ...
		   --validar saldo 
		   declare @elSaldo money
		   SELECT @elSaldo = SaldoCta from Cuenta where NumCta = @NumCta
		   
		   if(exists(select * from CuentaCAhorro where NumCta = @NumCta))
		   begin
		       if(@elSaldo < 0)
		       BEGIN
		           Rollback TRAN 
		   ...
		   BEGIN
		       --es tipo cuenta corriente
		       if(exists(select * from CuentaCorriente where NumCta = @NumCta AND @elSaldo <= MinimoCta))
		        BEGIN
		           Rollback TRAN 
		           return -4
		       END
